...
const linkAddr = "0x800AdfAD79A5dAEA572f8e0f5DE647a3F6b1ff89"
const daiAddr = "0x87ad70c3DD1776340094FDC444fBD5C2e490c492"
const compAddr = "0xf9C84F11efd5D11cE3a70020dC9f2B07B0D2019b"
const dexAddr = "0x20721d252d2e0308799C7c953daA30572BA33250"
...
$("#swap-box").submit(async (e)=>{
  ...
  try{buyMode ? await buyToken() : await sellToken()
  } catch (err){
    alert(err.message); 
  }
...
async function getPrice(){
  ...
  //apiからデータ(json)を取得するときにfetchを使う
  //javascriptではjsonファイルは使えないのでオブジェクトに直す必要がある(.json()をつける)
...
async function updateOutput(input){
  ...
  switch(token){
    case "COMP":
      output = buyMode ? input / priceData.compEth : input * priceData.compEth; //条件 (三項) 演算子
      break;
    case "LINK":
...
async function checkBalance(input){
  ...
  const balance = parseFloat(web3.utils.fromWei(balanceRaw, "ether")) //weiからethに変換

  if(balance >= input){
    $(".btn.swap").removeClass("disabled"); //disabledというclassをremoveする
    $(".btn.swap").html("Swap");//"enter the amount"の表記をswapに変える
  }else{
    $(".btn.swap").addClass("disabled");
...
function buyToken(){
  const tokenAddr = tokenInst._address; //tokenのコントラクトアドレス取得
